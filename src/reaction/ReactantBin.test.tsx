...
it('renders a chip per species and fires onRemove', async () => {
  ...
  render(<ReactantBin label="A" species={[z('Na')]} active qty={null} compound={null} reactant={null}
...
it('shows the TM charge picker for an unresolved transition metal and fires onPickCharge', async () => {
  ...
  render(<ReactantBin label="B" species={[z('Cu', { isTransition: true, oxidationStates: [1, 2] })]} active={false} qty={null} compound={null} reactant={null}
...
it('fires onActivate when the bin is clicked', async () => {
  ...
  render(<ReactantBin label="A" species={[]} active={false} qty={null} compound={null} reactant={null}
...
it('renders a single compound chip when compound is set', () => {
  render(<ReactantBin label="A" species={[z('Mg', { derivedCharge: 2 }), z('Cl', { derivedCharge: -1 })]}
    active qty={null} compound="MgCl₂" reactant={null}
...
it('renders the Ar/Mr mass line when a reactant is provided', () => {
  render(<ReactantBin label="A" species={[z('C')]} active qty={null} compound={null}
    reactant={{ formula: 'CO₂', molarMass: 44.01, massKind: 'Mr' }}
    onActivate={noop} onRemove={noop} onPickCharge={noop} onQty={noop} />);
  expect(screen.getByText('Mr')).toBeInTheDocument();
  expect(screen.getByText(/44\.01 g\/mol/)).toBeInTheDocument();
});
